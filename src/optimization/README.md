...
ACO_R (a Gaussian-archive variant designed specifically to *avoid*
discretization, used in an earlier version of this project — see
`src/optimization/README.md`'s history note). The trade-off: parameters
snap to one of `num_bins` levels instead of varying continuously,
capping precision. Expect ACO to plateau at a slightly worse cost than
PSO on this kind of smooth numeric landscape — that's the honest cost
of using "real" ACO here rather than a method built for continuous
domains.
